...
# create a different object for each file in file_names BEAUTIFUL, AMAZING
...
# mind that when using merge, if you have a 'Sites' (in this case) mistyped or something the dimensions will change, so that's a problem. Always check that the number of rows remains the same.
...
comm_df <- tidyr::pivot_longer(comm, cols = 2:ncol(comm),
                               names_to = "TaxCode",
                               values_to = "Abundance")

# taking something wider to make something longer. Were taking the object comm (community), taking only the second row on to the end. Take a look at head(comm) and View(comm_df) to understand whats happening better.
...
head(comm_sp)
# same as: dplyr::left_join(comm_df, splist, by = "TaxCode) # we could use right_join() r full_join() or whatever because the tables are perfect and complete
